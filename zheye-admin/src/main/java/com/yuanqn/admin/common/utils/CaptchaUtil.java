...
public class CaptchaUtil {
    ...
    private static RedisHelper redisHelper = (RedisHelper) SpringContextUtil.getBean("redisHelper");
    ...
    public static boolean verify(String code, HttpServletRequest request) {
        ...
        String key = Constant.REDIS_NAMESPACE + ":" + Constant.CODE_PREFIX + session.getId();
        String sessionCode = "";
        try {
            sessionCode = redisHelper.getString(key);
    ...
    private static void outCaptcha(int width, int height, int len, Font font, int cType, Integer vType, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            redisHelper.setString(key, 120000, code);
        } catch (GlobalException e) {
            log.error("保存验证码异常", e);
        }
